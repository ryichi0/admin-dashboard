...
export default function SidebarItems({ sidebarItem, sidebarOpen, i, itemIndex, setItemIndex }) {
  ...
  return (
    <li className="group relative">
      <button
        onClick={sidebarOpen ? () => setItemIndex(i) : null}
        ...
        {sidebarOpen &&
          <>
            ...
            <SvgChevronLeft className={`absolute left-2 transition-all ${itemIndex === i && "-rotate-90 stroke-light!"}`} />
          </>}
      </button>

      {sidebarOpen ?
        <ul className={`${itemIndex === i ? "max-h-80 my-4" : "max-h-0"} transition-all ms-16 flex flex-col gap-1.5 overflow-hidden`}>
